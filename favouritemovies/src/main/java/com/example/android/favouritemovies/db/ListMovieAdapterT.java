...
public class ListMovieAdapterT extends RecyclerView.Adapter<ListMovieAdapterT.MovieViewHolder> {
    private Cursor mCursor;
    private Context mcontext;
    private ArrayList<Movie> mData = new ArrayList<>();

    public ListMovieAdapterT(MovieFragmentT movieFragmentT) {
    }
    ...
    public void onBindViewHolder(@NonNull MovieViewHolder movieViewHolder, int position) {
        if (mCursor.moveToPosition(position)) {
            movieViewHolder.textViewTitle.setText(mCursor.getString(
                    mCursor.getColumnIndexOrThrow(TaskEntry.COLUMN_NAME)));
            movieViewHolder.textViewDateReleased.setText(mCursor.getString(
                    mCursor.getColumnIndexOrThrow(TaskEntry.COLUMN_DESC)));

            movieViewHolder.textViewDescription.setText(mCursor.getString(
                    mCursor.getColumnIndexOrThrow(TaskEntry.COLUMN_RELEASE)));
            String url_image =  mCursor.getString(
                    mCursor.getColumnIndexOrThrow(TaskEntry.COLUMN_MOVIE));
